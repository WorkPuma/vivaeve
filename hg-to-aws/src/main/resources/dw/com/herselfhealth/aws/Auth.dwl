 ...
 */
fun generateSecureAWSHeaders(method: String, region: String, service: String, uri: String, queryParams: String, request,  accessKey: String, secretKey: String): Object =
	do {
		var today = now() >> "GMT"
		var amz_date = today as String {format: "yyyyMMdd'T'HHmmss'Z'"}
		var date_stamp = today as String {format: "yyyyMMdd"}
		var requestHash = hash(request.^raw)
		var headers = {
			'host': '$(service).$(region).amazonaws.com',
			'x-amz-date': amz_date,
			'x-amz-content-sha256': requestHash
		}
		---
		headers ++ {
			"Authorization": computeAuthorizationHeader(amz_date, date_stamp, region, service, method, uri, queryParams, headers, requestHash, accessKey, secretKey)
		}
	}
 ...
 */
fun canoncialRequest(method: String, uri: String, querystring: String, headers: Object, requestHash: String) =
 ...
 */
fun computeAuthorizationHeader(amz_date: String, date_stamp: String, region: String, service: String, method: String, uri: String, queryString: String, headers: Object, requestHash: String, accessKey: String, secretKey: String) =
	do {
		var cr = canoncialRequest(method, uri, queryString, headers, requestHash)
